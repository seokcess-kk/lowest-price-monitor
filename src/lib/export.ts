...
export function exportToCSV(data: ExportRow[], filename: string): void {
  const header = '날짜,브랜드,상품명,사방넷코드,채널,가격,스토어명';
  const rows = data.map(
    (row) =>
      `${row.date},"${row.brandName ?? ''}","${row.productName}","${row.sabangnetCode ?? ''}",${row.channel},${row.price},"${row.storeName || ''}"`
  );
...
// xlsx 라이브러리(약 600KB)는 사용 시점에만 로드한다.
// 모듈 정적 import 시 모든 페이지 초기 번들에 포함돼 라우팅 체감 속도가 느려진다.
 ...
 *
 * 헤더(14열): 브랜드 / 상품명 / 사방넷코드 / 쿠팡 / 스토어 / 링크 / 네이버 / 스토어 / 링크 / 다나와 / 스토어 / 링크 / 최저가 / 최저채널
 * "스토어"·"링크"가 채널마다 반복되므로 객체 키 방식 대신 aoa_to_sheet로 직접 행 배열을 구성.
 * 쿠팡은 스토어명이 별도로 노출되지 않으므로 빈 문자열.
 */
export async function exportSnapshotToExcel(
  ...
  // 링크 셀 hyperlink 변환 — 브랜드 컬럼이 1번째 자리에 들어가 링크 컬럼은 +1씩 이동 (5/8/11)
  const linkColumns = [5, 8, 11];
